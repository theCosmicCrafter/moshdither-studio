...
//#region ../mosh-engine/src/adapters/MosherAdapter.ts
var MosherAdapter = class {
	pythonPath;
	moshCliPath;
	constructor(pythonPath, moshCliPath, _ffglitchPath, _ffmpegPath) {
	...
	*/
	async applyMosh(inputVideo, outputVideo, params) {
		const tmpDir = node_os.default.tmpdir();
		const configPath = node_path.default.join(tmpDir, `mosh_config_${Date.now()}.json`);
		try {
			const config = {
			...
			await node_fs.promises.writeFile(configPath, JSON.stringify(config, null, 2));
			...
			await new Promise((resolve, reject) => {
				...
				proc.stdout.on("data", (data) => console.log(`[mosh] ${data.toString().trim()}`));
				proc.stderr.on("data", (data) => console.error(`[mosh error] ${data.toString().trim()}`));
				proc.on("close", (code) => {
					if (code === 0) resolve();
					else reject(/* @__PURE__ */ new Error(`mosh_cli.py exited with code ${code}`));
				});
			});
			try {
				await node_fs.promises.access(outputVideo);
				return outputVideo;
			} catch {
				throw new Error(`Output file was not created by the mosh pipeline: ${outputVideo}`);
			}
		} finally {
			await node_fs.promises.unlink(configPath).catch(() => {});
		}
	}
	/**
	* Backward compatibility wrappers
	*/
	async moshClassic(inputVideo, outputVideo, intensity = .8) {
		return this.applyMosh(inputVideo, outputVideo, {
			mode: "classic",
			intensity
		});
	}
	async moshShuffle(inputVideo, outputVideo, intensity = .5) {
		return this.applyMosh(inputVideo, outputVideo, {
			mode: "shuffle",
			intensity
		});
	}
	async moshSwapVectors(targetVideo, motionVideo, outputVideo) {
...
//#endregion
//#region ../mosh-engine/src/adapters/DitherAdapter.ts
function runSpawn(cmd, args) {
	return new Promise((resolve, reject) => {
		const proc = (0, node_child_process.spawn)(cmd, args);
		proc.stdout.on("data", (data) => console.log(`[dither] ${data.toString().trim()}`));
		proc.stderr.on("data", (data) => console.error(`[dither error] ${data.toString().trim()}`));
		proc.on("close", (code) => {
			if (code === 0) resolve(code);
			else reject(/* @__PURE__ */ new Error(`dither_cli.py exited with code ${code}`));
		});
	});
}
var DitherAdapter = class {
	pythonPath;
	ditherCliPath;
	constructor(pythonPath, ditherCliPath) {
		this.pythonPath = pythonPath;
		this.ditherCliPath = ditherCliPath;
	}
	async applyDither(inputPath, params) {
		const ext = node_path.default.extname(inputPath);
		const outputPath = node_path.default.join(node_os.default.tmpdir(), `dither_out_${Date.now()}${ext}`);
		const configPath = node_path.default.join(node_os.default.tmpdir(), `dither_config_${Date.now()}.json`);
		let finalPaletteSource = params.paletteSource;
		if (params.paletteSource === "gameboy") finalPaletteSource = "gb_dmg_palette";
		else if (params.paletteSource === "cga") finalPaletteSource = "cga_palette1";
		else if (params.paletteSource === "bw") finalPaletteSource = "1bit_monitor_glow_2colors";
		const parameters = {};
		if (params.ditherMode === "bayer") parameters.size = typeof params.matrixSize === "number" ? `${params.matrixSize}x${params.matrixSize}` : params.matrixSize || "4x4";
			...
			parameters.scale = params.scale ?? 1;
			...
			parameters.var_threshold = params.varThreshold ?? 300;
			parameters.window_radius = params.windowRadius ?? 1;
		} else if (params.ditherMode === "hybrid") {
			parameters.lum_factor = params.lumFactor ?? 1;
			parameters.col_factor = params.colFactor ?? .2;
		} else if (params.ditherMode === "halftone") {
			parameters.cell_size = params.dotSize ?? 8;
			parameters.angle = params.angle ?? 45;
			parameters.dot_gain = params.dotGain ?? 1;
			parameters.min_dot_size = params.minDotSize ?? 0;
			parameters.max_dot_size = params.maxDotSize ?? 1;
			parameters.shape = params.shape || "circle";
			parameters.sharpness = params.sharpness ?? 1.5;
		} else if (params.ditherMode === "ostromoukhov") parameters.serpentine = params.serpentine ? "true" : "false";
		const config = {
			input: inputPath,
			output: outputPath,
			dithering: {
		...
		try {
			...
			const args = isStandalone ? [configPath] : [this.ditherCliPath, configPath];
			console.log(`Starting dither backend: ${cmd} ${args.join(" ")}`);
			await runSpawn(cmd, args);
			if (!await node_fs.promises.access(outputPath).then(() => true).catch(() => false)) throw new Error(`Output file was not created: ${outputPath}`);
			return outputPath;
		} finally {
			try {
				await node_fs.promises.unlink(configPath);
			} catch (e) {}
		}
	}
};
...
*/
var LOGS_DIR = node_path.default.join(node_os.default.homedir(), ".moshdither", "logs");
var RETENTION_DAYS = 30;
var MAX_LOG_FILES = 200;
var sessionLogPath = "";
var runsPath = "";
var latestPath = "";
var logStream = null;
...
function sanitizeText(text) {
	let out = text;
	for (const pattern of SENSITIVE_PATTERNS) out = out.replace(pattern.regex, pattern.replacement);
	out = out.replace(/C:\\Users\\[^\\]+/gi, "C:\\Users\\[USER]");
	out = out.replace(/\/home\/[^/]+/g, "/home/[USER]");
	return out;
}
function ensureLogDir() {
	if (!node_fs.default.existsSync(LOGS_DIR)) node_fs.default.mkdirSync(LOGS_DIR, { recursive: true });
}
...
function rotateOldLogs() {
	try {
		...
		const files = node_fs.default.readdirSync(LOGS_DIR);
		const logCandidates = [];
		for (const f of files) {
			const full = node_path.default.join(LOGS_DIR, f);
			try {
				const st = node_fs.default.statSync(full);
				if (!st.isFile()) continue;
				if (st.mtimeMs < cutoff) {
					node_fs.default.unlinkSync(full);
					continue;
				}
				if (f.startsWith("main-") && f.endsWith(".log")) logCandidates.push({
					full,
					mtimeMs: st.mtimeMs
				});
			} catch {}
		...
		if (logCandidates.length > MAX_LOG_FILES) {
			...
			const toDelete = logCandidates.slice(0, logCandidates.length - MAX_LOG_FILES);
			for (const item of toDelete) try {
				node_fs.default.unlinkSync(item.full);
			} catch {}
		}
	} catch {}
}
...
function appendRunEvent(evt) {
	try {
		node_fs.default.appendFileSync(runsPath, `${JSON.stringify(evt)}\n`, "utf8");
	} catch {}
}
var origConsole = {
	log: console.log,
...
function installLogger() {
	...
	sessionLogPath = node_path.default.join(LOGS_DIR, `main-${sessionId}.log`);
	runsPath = node_path.default.join(LOGS_DIR, "runs.jsonl");
	latestPath = node_path.default.join(LOGS_DIR, "latest.txt");
	try {
		node_fs.default.writeFileSync(latestPath, `${sessionLogPath}\n`, "utf8");
	} catch {}
	logStream = node_fs.default.createWriteStream(sessionLogPath, { flags: "a" });
	appendRunEvent({
		type: "start",
	...
	interceptConsole();
	const crashMarkerPath = node_path.default.join(LOGS_DIR, "crash-marker.json");
	process.on("uncaughtException", (err) => {
		writeLine("FATAL", ["Uncaught exception:", err]);
		try {
			node_fs.default.writeFileSync(crashMarkerPath, JSON.stringify({
				ts: (/* @__PURE__ */ new Date()).toISOString(),
				sessionId,
				pid: process.pid,
	...
	process.on("unhandledRejection", (reason) => {
		writeLine("ERROR", ["Unhandled rejection:", reason instanceof Error ? reason : new Error(String(reason))]);
	});
	console.log("[Logger] Session logs:", sessionLogPath);
}
function writeRendererLog(level, message, details) {
	writeLine(level, [`[Renderer] ${message}`, details]);
}
function checkCrashMarker() {
	const crashMarkerPath = node_path.default.join(LOGS_DIR, "crash-marker.json");
	try {
		if (node_fs.default.existsSync(crashMarkerPath)) {
			const raw = node_fs.default.readFileSync(crashMarkerPath, "utf8");
			const info = JSON.parse(raw);
			node_fs.default.unlinkSync(crashMarkerPath);
			return {
				crashed: true,
				info
			};
...
var require_utils$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	module.exports.checkPath = function checkPath(pth) {
		if (process.platform === "win32") {
			if (/[<>:"|?*]/.test(pth.replace(path$29.parse(pth).root, ""))) {
...
var require_stat = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		...
		util$2.callbackify(getStats)(src, dest, opts, (err, stats) => {
			...
			if (destStat) {
				if (areIdentical(srcStat, destStat)) {
					const srcBaseName = path$28.basename(src);
					const destBaseName = path$28.basename(dest);
	...
	function checkParentPaths(src, srcStat, dest, funcName, cb) {
		const srcParent = path$28.resolve(path$28.dirname(src));
		const destParent = path$28.resolve(path$28.dirname(dest));
		if (destParent === srcParent || destParent === path$28.parse(destParent).root) return cb();
	...
	function checkParentPathsSync(src, srcStat, dest, funcName) {
		const srcParent = path$28.resolve(path$28.dirname(src));
		const destParent = path$28.resolve(path$28.dirname(dest));
		if (destParent === srcParent || destParent === path$28.parse(destParent).root) return;
		let destStat;
		try {
	...
	function isSrcSubdir(src, dest) {
		const srcArr = path$28.resolve(src).split(path$28.sep).filter((i) => i);
		const destArr = path$28.resolve(dest).split(path$28.sep).filter((i) => i);
...
var require_rimraf = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function rmkids(p, options, cb) {
		...
		options.readdir(p, (er, files) => {
			...
			files.forEach((f) => {
				rimraf(path$25.join(p, f), options, (er) => {
					if (errState) return;
	...
	function rmkidsSync(p, options) {
		assert(p);
		assert(options);
		options.readdirSync(p).forEach((f) => rimrafSync(path$25.join(p, f), options));
		if (isWindows) {
...
var require_empty = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var emptyDir = u(async function emptyDir(dir) {
		...
		return Promise.all(items.map((item) => remove.remove(path$24.join(dir, item))));
	});
...
var require_file = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function createFile(file, callback) {
		...
		fs.stat(file, (err, stats) => {
			...
			const dir = path$23.dirname(file);
	...
	function createLink(srcpath, dstpath, callback) {
		...
		fs.lstat(dstpath, (_, dstStat) => {
			fs.lstat(srcpath, (err, srcStat) => {
				...
				const dir = path$22.dirname(dstpath);
...
var require_symlink_paths = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	*/
	function symlinkPaths(srcpath, dstpath, callback) {
		if (path$21.isAbsolute(srcpath)) return fs.lstat(srcpath, (err) => {
			if (err) {
			...
			const dstdir = path$21.dirname(dstpath);
			const relativeToDst = path$21.join(dstdir, srcpath);
			return pathExists(relativeToDst, (err, exists) => {
				...
				if (exists) return callback(null, {
					...
					return callback(null, {
						toCwd: srcpath,
						toDst: path$21.relative(dstdir, srcpath)
					});
				});
			});
		}
	...
	function move(src, dest, opts, cb) {
		...
		stat.checkPaths(src, dest, "move", opts, (err, stats) => {
			...
			stat.checkParentPaths(src, srcStat, dest, "move", (err) => {
				...
				mkdirp(path$18.dirname(dest), (err) => {
					if (err) return cb(err);
...
var require_move_sync = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function moveSync(src, dest, opts) {
		...
		if (!isParentRoot(dest)) mkdirpSync(path$17.dirname(dest));
...
var require_DownloadedUpdateHelper = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var DownloadedUpdateHelper = class {
		...
		get cacheDirForPendingUpdate() {
			return path$16.join(this.cacheDir, "pending");
		}
		...
		*/
		async getValidCachedUpdateFile(fileInfo, logger) {
			...
			const updateFile = path$16.join(this.cacheDirForPendingUpdate, cachedInfo.fileName);
		...
		getUpdateInfoFile() {
			return path$16.join(this.cacheDirForPendingUpdate, "update-info.json");
		}
	};
	...
	async function createTempUpdateFile(name, cacheDir, log) {
		let nameCounter = 0;
		let result = path$16.join(cacheDir, name);
		for (let i = 0; i < 3; i++) try {
			...
			result = path$16.join(cacheDir, `${nameCounter++}-${name}`);
		}
		return result;
	}
}));
...
var require_AppAdapter = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function getAppCacheDir() {
		...
		let result;
		if (process.platform === "win32") result = process.env["LOCALAPPDATA"] || path$15.join(homedir, "AppData", "Local");
		else if (process.platform === "darwin") result = path$15.join(homedir, "Library", "Caches");
		else result = process.env["XDG_CACHE_HOME"] || path$15.join(homedir, ".cache");
		return result;
	}
}));
//#endregion
...
var require_ElectronAppAdapter = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var ElectronAppAdapter = class {
		...
		get appUpdateConfigPath() {
			return this.isPackaged ? path$14.join(process.resourcesPath, "app-update.yml") : path$14.join(this.app.getAppPath(), "dev-app-update.yml");
		}
		get userDataPath() {
	...
	var PrivateGitHubProvider = class extends GitHubProvider_1.BaseGitHubProvider {
		...
		resolveFiles(updateInfo) {
			return (0, Provider_1.getFileList)(updateInfo).map((it) => {
				const name = path$13.posix.basename(it.url).replace(/ /g, "-");
	...
	exports.AppUpdater = class AppUpdater extends events_1.EventEmitter {
		...
		*/
		...
		async getOrCreateStagingUserId() {
			const file = path$12.join(this.app.userDataPath, ".updaterId");
			try {
		...
		async getOrCreateDownloadHelper() {
			...
			if (result == null) {
				...
				const cacheDir = path$12.join(this.app.baseCachePath, dirName || this.app.name);
		...
		async executeDownload(taskOptions) {
			...
			function getCacheUpdateFileName() {
				...
				if (urlPath.toLowerCase().endsWith(`.${taskOptions.fileExtension.toLowerCase()}`)) return path$12.basename(urlPath);
				else return path$12.basename(taskOptions.fileInfo.info.url);
			}
			const downloadedUpdateHelper = await this.getOrCreateDownloadHelper();
			const cacheDir = downloadedUpdateHelper.cacheDirForPendingUpdate;
			await (0, fs_extra_1.mkdir)(cacheDir, { recursive: true });
			const updateFileName = getCacheUpdateFileName();
			let updateFile = path$12.join(cacheDir, updateFileName);
			const packageFile = packageInfo == null ? null : path$12.join(cacheDir, `package-${version}${path$12.extname(packageInfo.path) || ".7z"}`);
			const done = async (isSaveCache) => {
				...
				const currentBlockMapFile = path$12.join(cacheDir, "current.blockmap");
				if (await (0, fs_extra_1.pathExists)(currentBlockMapFile)) await (0, fs_extra_1.copyFile)(currentBlockMapFile, path$12.join(downloadedUpdateHelper.cacheDir, "current.blockmap"));
		...
		async differentialDownloadInstaller(fileInfo, downloadUpdateOptions, installerPath, provider, oldInstallerFileName) {
			try {
				...
				const downloadOptions = {
					newUrl: fileInfo.url,
					oldFile: path$12.join(this.downloadedUpdateHelper.cacheDir, oldInstallerFileName),
					logger: this._logger,
				...
				const saveBlockMapToCacheDir = async (blockMapData, cacheDir) => {
					const blockMapFile = path$12.join(cacheDir, "current.blockmap");
	...
	var BaseUpdater = class extends AppUpdater_1.AppUpdater {
		...
		*/
		sanitizeEnvPath(envPath) {
			return envPath.split(path$11.delimiter).filter((dir) => path$11.isAbsolute(dir)).join(path$11.delimiter);
		}
...
var require_AppImageUpdater = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var AppImageUpdater = class extends BaseUpdater_1.BaseUpdater {
		...
		doInstall(options) {
			...
			if (!path$10.isAbsolute(appImageFile) || appImageFile.includes("\0")) throw (0, builder_util_runtime_1.newError)(`APPIMAGE env is not a valid absolute path: "${appImageFile}"`, "ERR_UPDATER_OLD_FILE_NOT_FOUND");
			(0, fs_1$1.unlinkSync)(appImageFile);
			let destination;
			const existingBaseName = path$10.basename(appImageFile);
			const installerPath = this.installerPath;
			if (installerPath == null) {
				this.dispatchError(/* @__PURE__ */ new Error("No update filepath provided, can't quit and install"));
				return false;
			}
			if (path$10.basename(installerPath) === existingBaseName || !/\d+\.\d+\.\d+/.test(existingBaseName)) destination = appImageFile;
			else destination = path$10.join(path$10.dirname(appImageFile), path$10.basename(installerPath));
		...
		async doDownloadUpdate(downloadUpdateOptions) {
			...
			return this.executeDownload({
				...
				task: async (destinationFile, downloadOptions) => {
					const cachedUpdateFilePath = path$9.join(this.downloadedUpdateHelper.cacheDir, CURRENT_MAC_APP_ZIP_FILE_NAME);
		...
		return new Promise((resolve, reject) => {
			...
			(0, child_process_1.execFile)(...preparePowerShellExec(`"Get-AuthenticodeSignature -LiteralPath '${tempUpdateFile}' | ConvertTo-Json -Compress"`, 20 * 1e3), (error, stdout, stderr) => {
				...
				try {
					...
					if (data.Status === 0) {
						try {
							const normlaizedUpdateFilePath = path$8.normalize(data.Path);
							const normalizedTempUpdateFile = path$8.normalize(unescapedTempUpdateFile);
		...
		*/
		...
		doInstall(options) {
			...
			const callUsingElevation = () => {
				this.spawnLog(path$7.join(process.resourcesPath, "elevate.exe"), [installerPath].concat(args)).catch((e) => this.dispatchError(e));
			};
		...
		async differentialDownloadWebPackage(downloadUpdateOptions, packageInfo, packagePath, provider) {
			...
			try {
				const downloadOptions = {
					...
					oldFile: path$7.join(this.downloadedUpdateHelper.cacheDir, builder_util_runtime_1.CURRENT_APP_PACKAGE_FILE_NAME),
					logger: this._logger,
					newFile: packagePath,
...
var import_main = (/* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function doLoadAutoUpdater() {
		if (process.platform === "win32") _autoUpdater = new (require_NsisUpdater()).NsisUpdater();
			...
			try {
				const identity = path$6.join(process.resourcesPath, "package-type");
...
function initAutoUpdater(win) {
	...
	import_main.autoUpdater.on("update-not-available", () => {
	...
	electron.ipcMain.handle("update:check-now", async () => {
		try {
			return await import_main.autoUpdater.checkForUpdatesAndNotify() ?? null;
		} catch (err) {
			writeRendererLog("error", "Manual update check failed", { message: err instanceof Error ? err.message : String(err) });
			return null;
		}
	});
	electron.ipcMain.handle("update:install-now", () => {
		import_main.autoUpdater.quitAndInstall(false, true);
	});
	setTimeout(() => {
		import_main.autoUpdater.checkForUpdatesAndNotify().catch((err) => {
			writeRendererLog("error", "Initial update check failed", { message: err instanceof Error ? err.message : String(err) });
		});
	}, 5e3);
}
function sendToRenderer(channel, data) {
	if (updateWindow && !updateWindow.isDestroyed()) updateWindow.webContents.send(channel, data);
}
//#endregion
//#region electron/pythonBackend.ts
/**
* Python Backend Client
* Spawns the Python RPC server and provides typed helpers to call it.
*/
var _pythonPort = null;
var _pythonToken = null;
var _pythonProcess = null;
var _stdoutListeners = [];
function addStdoutListener(listener) {
	_stdoutListeners.push(listener);
}
var RPC_TIMEOUT = 6e5;
function getProjectRoot() {
	if (process.env.VITE_DEV_SERVER_URL) return node_path.default.resolve(__dirname, "../../..");
	return node_path.default.join(electron.app.getAppPath(), "../..");
}
/**
...
function startPythonBackend() {
	return new Promise((resolve, reject) => {
		...
		const pythonScript = node_path.default.join(projectRoot, "packages/python-backend/main.py");
		const venvPython = node_path.default.join(projectRoot, "packages/python-backend/venv/Scripts/python.exe");
		const pythonExe = process.platform === "win32" && node_fs.default.existsSync(venvPython) ? venvPython : process.platform === "win32" ? "python" : "python3";
		const env = {
			...process.env,
		...
		const proc = (0, node_child_process.spawn)(pythonExe, [pythonScript, "0"], {
			cwd: projectRoot,
			env,
			stdio: [
				"ignore",
		...
		proc.stdout.on("data", (data) => {
			...
			for (const listener of _stdoutListeners) try {
				listener(text);
			} catch (err) {
			...
			if (portMatch) _pythonPort = parseInt(portMatch[1], 10);
			if (tokenMatch) _pythonToken = tokenMatch[1];
		...
		proc.on("error", (err) => {
			reject(/* @__PURE__ */ new Error(`Failed to start Python backend: ${err.message}`));
		});
		proc.on("exit", (code) => {
			if (code !== 0 && code !== null) {
		...
		const startupTimer = setTimeout(() => {
			if (!_pythonPort) reject(/* @__PURE__ */ new Error("Python backend failed to start within 30s"));
		}, 3e4);
...
*/
async function pythonRpcCall(method, params) {
	if (!_pythonPort || !_pythonToken) throw new Error("Python backend not started");
	...
	try {
		...
		if (!res.ok) throw new Error(`Python RPC HTTP ${res.status}: ${await res.text()}`);
		const json = await res.json();
		if (json.error) throw new Error(`Python RPC error: ${json.error.message}`);
		return json.result;
	} catch (err) {
		clearTimeout(timeout);
		const errorMsg = err instanceof Error ? err.message : String(err);
		console.error(`[PythonBackend] RPC call to ${method} failed:`, errorMsg, "Port:", _pythonPort, "Token length:", _pythonToken ? _pythonToken.length : "null");
		throw err;
	}
}
...
//#endregion
//#region electron/frameCacheMain.ts
/**
...
*/
var CACHE_DIR = node_path.default.join(node_os.default.homedir(), ".moshdither", "cache");
var MAX_CACHE_SIZE_MB = 1024;
var cachedTotalSizeBytes = 0;
function hashKey(input) {
	return node_crypto.default.createHash("sha256").update(input).digest("hex").slice(0, 16);
}
function getCachePath(key) {
	const h = hashKey(key);
	return node_path.default.join(CACHE_DIR, `${h}.png`);
}
function getMetaPath(key) {
	const h = hashKey(key);
	return node_path.default.join(CACHE_DIR, `${h}.json`);
}
function ensureCacheDir() {
	if (!node_fs.default.existsSync(CACHE_DIR)) node_fs.default.mkdirSync(CACHE_DIR, { recursive: true });
}
function readMeta(key) {
	const metaPath = getMetaPath(key);
	if (!node_fs.default.existsSync(metaPath)) return null;
	try {
		return JSON.parse(node_fs.default.readFileSync(metaPath, "utf-8"));
	} catch {
		return null;
	}
}
function writeMeta(entry) {
	ensureCacheDir();
	node_fs.default.writeFileSync(getMetaPath(entry.key), JSON.stringify(entry), "utf-8");
}
function getCachedFrame(key) {
	const cachePath = getCachePath(key);
	if (!node_fs.default.existsSync(cachePath)) return null;
...
function setCachedFrame(key, data) {
	ensureCacheDir();
	const cachePath = getCachePath(key);
	node_fs.default.writeFileSync(cachePath, data);
	writeMeta({
		key,
		createdAt: Date.now(),
	...
	if (cachedTotalSizeBytes > MAX_CACHE_SIZE_MB * 1024 * 1024) enforceSizeLimit();
}
function enforceSizeLimit() {
	const entries = [];
	for (const file of node_fs.default.readdirSync(CACHE_DIR)) if (file.endsWith(".json")) try {
		const meta = JSON.parse(node_fs.default.readFileSync(node_path.default.join(CACHE_DIR, file), "utf-8"));
		entries.push(meta);
	} catch {}
	const totalSize = entries.reduce((sum, e) => sum + e.sizeBytes, 0);
	cachedTotalSizeBytes = totalSize;
	...
	for (const entry of entries) {
		...
		if (node_fs.default.existsSync(cachePath)) {
			node_fs.default.unlinkSync(cachePath);
			cachedTotalSizeBytes -= entry.sizeBytes;
		}
		if (node_fs.default.existsSync(metaPath)) node_fs.default.unlinkSync(metaPath);
		toEvict -= entry.sizeBytes;
	}
}
function clearFrameCache() {
	if (!node_fs.default.existsSync(CACHE_DIR)) return;
	for (const file of node_fs.default.readdirSync(CACHE_DIR)) if (file.endsWith(".png") || file.endsWith(".json")) node_fs.default.unlinkSync(node_path.default.join(CACHE_DIR, file));
	cachedTotalSizeBytes = 0;
}
function getCacheStats() {
	...
	let totalSize = 0;
	let count = 0;
	for (const file of node_fs.default.readdirSync(CACHE_DIR)) if (file.endsWith(".png")) {
		count++;
		totalSize += node_fs.default.statSync(node_path.default.join(CACHE_DIR, file)).size;
	}
	return {
		entries: count,
...
//#endregion
//#region src/utils/watermark.ts
/**
* Convert hex/rgb color name to FFmpeg drawtext color format.
*/
function toDrawtextColor(color) {
	return {
		white: "white",
		black: "black",
		red: "red",
...
*/
function getOverlayCoords(position, type) {
	if (type === "text") switch (position) {
...
function buildDrawtextFilter(settings) {
	...
	const alpha = Math.round(settings.opacity * 255).toString(16).padStart(2, "0");
	let filter = `drawtext=text='${escapeDrawtext(settings.text)}':x=${x}:y=${y}:fontsize=${settings.fontSize}:fontcolor=${color}@${alpha}`;
	if (settings.fontPath) filter += `:fontfile=${settings.fontPath}`;
	return filter;
}
/**
...
*/
function buildImageOverlayFilter(settings) {
	if (!settings.enabled || settings.type !== "image" || !settings.imagePath) return {
		filter: null,
		extraInputIndex: null
	};
	const { x, y } = getOverlayCoords(settings.position, "image");
	const alpha = settings.opacity;
	const scaleExpr = settings.scale > 0 ? `scale=-1:${settings.scale}*ih/100` : "";
	...
	if (scaleExpr) {
		...
		if (opacityExpr) filter += `[wm];[wm]${opacityExpr}[wm2];[0:v][wm2]${overlayExpr}`;
		else filter += `[wm];[0:v][wm]${overlayExpr}`;
	} else if (opacityExpr) filter += `[1:v]${opacityExpr}[wm];[0:v][wm]${overlayExpr}`;
	else filter += `[0:v][1:v]${overlayExpr}`;
	return {
		filter,
		extraInputIndex: 1
	};
...
*/
function buildWatermarkArgs(baseArgs, settings) {
	...
	if (settings.type === "text") {
		...
		if (vfIndex !== -1 && vfIndex + 1 < args.length) args[vfIndex + 1] = `${args[vfIndex + 1]},${drawtext}`;
		else {
			const insertIndex = args.findIndex((a) => a.endsWith(".mp4") || a.endsWith(".gif") || a.endsWith(".png") || a.endsWith(".jpg"));
			if (insertIndex !== -1) args.splice(insertIndex, 0, "-vf", drawtext);
			else args.push("-vf", drawtext);
		}
		return {
			args,
			extraInputs: []
	...
	if (!filter || extraInputIndex === null) return {
		args: baseArgs,
		extraInputs: []
	};
	const args = [...baseArgs];
	const firstInputIndex = args.indexOf("-i");
	if (firstInputIndex !== -1) args.splice(firstInputIndex + 2, 0, "-i", settings.imagePath);
	...
	if (fcIndex !== -1 && fcIndex + 1 < args.length) {
		...
		if (fcIndex === args.indexOf("-vf")) {
			...
			args[fcIndex + 1] = `[0:v]${existing}[v0];${filter}[v1];[v1]${existing.replace(/scale=trunc\(iw\/2\)\*2:trunc\(ih\/2\)\*2/, "")}[out]`;
		} else args[fcIndex + 1] = `${existing};${filter}`;
	} else {
		const insertIndex = args.findIndex((a) => a.endsWith(".mp4") || a.endsWith(".gif") || a.endsWith(".png") || a.endsWith(".jpg"));
		if (insertIndex !== -1) args.splice(insertIndex, 0, "-filter_complex", filter);
		else args.push("-filter_complex", filter);
	}
	if (!args.includes("-map")) {
		const outputIndex = args.findIndex((a) => a.endsWith(".mp4") || a.endsWith(".gif") || a.endsWith(".png") || a.endsWith(".jpg"));
		if (outputIndex !== -1) args.splice(outputIndex, 0, "-map", "[out]");
	}
	return {
		args,
		extraInputs: []
...
//#endregion
//#region node_modules/get-system-fonts/dist/recursiveWalk.js
var require_recursiveWalk = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	var fs$2 = require("fs");
	var util = require("util");
	var path$3 = require("path");
	var readdirAsync = util.promisify(fs$2.readdir);
	var statAsync = util.promisify(fs$2.stat);
	/**
	...
	*/
	async function recursiveWalk(baseDirs, extensions) {
		...
		await Promise.all(baseDirs.map(async (baseDir) => {
			const files = await recursiveWalkInternal(path$3.resolve(baseDir), new RegExp(`\\.${extensions.map((ext) => `(?:${ext})`).join("|")}$`, "i"));
			for (const file of files) results.add(file);
		}));
		return [...results];
	}
	...
	*/
	async function recursiveWalkInternal(baseDir, extensionRegex, maxDepth = 10) {
		...
		await Promise.all(entries.map(async (entry) => {
			const entryPath = path$3.join(baseDir, entry);
			let stats;
			try {
			...
			if (stats.isFile() && extensionRegex.test(entryPath)) results.push(entryPath);
			else if (stats.isDirectory()) results.push(...await recursiveWalkInternal(entryPath, extensionRegex, maxDepth - 1));
		}));
		return results;
	}
}));
//#endregion
//#region electron/environmentManager.ts
var import_dist = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	var path$2 = require("path");
	var os$1 = require("os");
	var recursiveWalk_1 = require_recursiveWalk();
	var directories = {
		win32: () => {
			const globalDir = path$2.join(process.env.WINDIR || "C:\\Windows", "Fonts");
			const appDataDir = "Microsoft\\Windows\\Fonts";
			let localDir;
			if (process.env.LOCALAPPDATA) localDir = path$2.join(process.env.LOCALAPPDATA, appDataDir);
			else if (process.env.APPDATA) localDir = path$2.join(process.env.APPDATA, "Local", appDataDir);
			else if (process.env.USERPROFILE) localDir = path$2.join(process.env.USERPROFILE, "AppData", "Local", appDataDir);
			if (localDir) return [globalDir, localDir];
			else return [globalDir];
		},
		darwin: () => {
			const home = os$1.homedir();
			return [
				...home ? [path$2.join(home, "/Library/Fonts")] : [],
				"/Library/Fonts",
		...
		linux: () => {
			...
			return [
				...
				...home ? [path$2.join(home, ".fonts"), path$2.join(home, ".local/share/fonts")] : []
			];
		}
	};
	/**
	...
	*/
	function getSystemFonts(options) {
		...
		if (!getDirs) throw new Error(`Unsupported platform: ${platform}`);
		const dirs = getDirs();
...
*/
var ENV_CONFIG_FILE = "env-config.json";
var CHECKPOINT_FILE = "install-checkpoint.json";
var VENV_DIR_NAME = "python-env";
var ASSETS_BIN = "assets/bin";
function checkpointPath() {
	return node_path.default.join(electron.app.getPath("userData"), CHECKPOINT_FILE);
}
function readCheckpoint() {
	try {
		const raw = node_fs.default.readFileSync(checkpointPath(), "utf-8");
...
function writeCheckpoint(cp) {
	try {
		node_fs.default.writeFileSync(checkpointPath(), JSON.stringify(cp, null, 2));
	} catch {}
}
function clearCheckpoint() {
	try {
		if (node_fs.default.existsSync(checkpointPath())) node_fs.default.unlinkSync(checkpointPath());
	} catch {}
}
function configPath() {
	return node_path.default.join(electron.app.getPath("userData"), ENV_CONFIG_FILE);
}
function loadEnvConfig() {
	try {
		const raw = node_fs.default.readFileSync(configPath(), "utf-8");
		const parsed = JSON.parse(raw);
		if (parsed.mode === "local" || parsed.mode === "system") return parsed;
	} catch {}
	return { mode: "unconfigured" };
}
function saveEnvConfig(mode, installState) {
	const payload = { mode };
	if (installState) payload.installState = installState;
	node_fs.default.writeFileSync(configPath(), JSON.stringify(payload, null, 2));
}
function userDataDir() {
	return electron.app.getPath("userData");
}
function venvDir() {
	return node_path.default.join(userDataDir(), VENV_DIR_NAME);
}
function venvPython() {
	const dir = venvDir();
	return process.platform === "win32" ? node_path.default.join(dir, "Scripts", "python.exe") : node_path.default.join(dir, "bin", "python");
}
function venvPip() {
	const dir = venvDir();
	return process.platform === "win32" ? node_path.default.join(dir, "Scripts", "pip.exe") : node_path.default.join(dir, "bin", "pip");
}
function bundledAssetDir(sub) {
	if (!electron.app.isPackaged) return node_path.default.resolve(__dirname, "../../../", ASSETS_BIN, sub);
	return node_path.default.join(process.resourcesPath, ASSETS_BIN, sub);
}
function fileExists(p) {
	try {
		node_fs.default.accessSync(p, node_fs.default.constants.F_OK);
		return true;
	} catch {
		return false;
	}
...
function which(bin) {
	return new Promise((resolve) => {
		(0, node_child_process.execFile)(process.platform === "win32" ? "where" : "which", [bin], (err, stdout) => {
			if (err) {
				resolve(void 0);
				return;
			}
			const first = stdout.trim().split(/\r?\n/)[0];
			resolve(first || void 0);
		});
	});
}
async function getEnvironmentStatus() {
	...
	const status = {
		...
		ffprobeOk: false,
		ffglitchOk: false,
		installInProgress: cfg.mode === "local" && cfg.installState === "in-progress",
		installCheckpoint: checkpoint ?? void 0
	};
	if (cfg.mode === "system") {
	...
	if (cfg.mode === "local") {
		...
		status.venvOk = node_fs.default.existsSync(venvDir());
		status.pipOk = fileExists(venvPip());
		const copiedFfmpegDir = bundledAssetDir("ffmpeg");
		const copiedFfglitchDir = bundledAssetDir("ffglitch");
		let ffmpeg = node_path.default.join(copiedFfmpegDir, `ffmpeg${exeSuffix()}`);
		let ffprobe = node_path.default.join(copiedFfmpegDir, `ffprobe${exeSuffix()}`);
		if (!node_fs.default.existsSync(ffmpeg)) {
			const repoRoot = findProjectRoot();
			ffmpeg = node_path.default.join(repoRoot, "assets", "bin", "ffmpeg-master-latest-win64-gpl", "bin", `ffmpeg${exeSuffix()}`);
			ffprobe = node_path.default.join(repoRoot, "assets", "bin", "ffmpeg-master-latest-win64-gpl", "bin", `ffprobe${exeSuffix()}`);
		}
		status.ffmpegPath = ffmpeg;
		status.ffprobePath = ffprobe;
		status.ffmpegOk = node_fs.default.existsSync(ffmpeg);
		status.ffprobeOk = node_fs.default.existsSync(ffprobe);
		let ffgac = node_path.default.join(copiedFfglitchDir, `ffgac${exeSuffix()}`);
		let ffedit = node_path.default.join(copiedFfglitchDir, `ffedit${exeSuffix()}`);
		if (!node_fs.default.existsSync(ffgac)) {
			const repoRoot = findProjectRoot();
			ffgac = node_path.default.join(repoRoot, "assets", "bin", "ffglitch-0.10.2-windows-x86_64", `ffgac${exeSuffix()}`);
			ffedit = node_path.default.join(repoRoot, "assets", "bin", "ffglitch-0.10.2-windows-x86_64", `ffedit${exeSuffix()}`);
		}
		status.ffgacPath = ffgac;
		status.ffeditPath = ffedit;
		status.ffglitchOk = node_fs.default.existsSync(ffgac) && node_fs.default.existsSync(ffedit);
		const repoRoot = findProjectRoot();
		status.moshCliPath = node_path.default.join(repoRoot, "packages/python-backend/mosh_cli.py");
		status.ditherCliPath = node_path.default.join(repoRoot, "references/dither_pie/dither_cli.py");
		if (cfg.installState === "complete") {
			status.installInProgress = false;
			clearCheckpoint();
		}
		if (cfg.installState === "complete" && (!status.ffmpegOk || !status.ffprobeOk || !status.ffglitchOk)) {
...
async function installLocalEnvironment(onProgress) {
	saveEnvConfig("local", "in-progress");
	const checkpoint = readCheckpoint();
	const completed = new Set(checkpoint?.completedSteps ?? []);
	function report(stepId, detail) {
	...
	function markDone(stepId) {
		...
		const step = INSTALL_STEPS.find((s) => s.id === stepId);
		writeCheckpoint({
			startedAt: checkpoint?.startedAt ?? (/* @__PURE__ */ new Date()).toISOString(),
	...
	if (!systemPython) {
		clearCheckpoint();
		throw new Error("Python is required but was not found on your system.\nPlease install Python 3.9+ from https://python.org and try again.");
	}
	markDone("check-python");
	if (!completed.has("create-venv")) {
		report("create-venv");
		const venv = venvDir();
		if (!node_fs.default.existsSync(venv)) await runCommand(systemPython, [
			"-m",
			"venv",
			venv
		]);
		...
		if (!fileExists(py)) {
			clearCheckpoint();
			throw new Error(`Virtual environment created but python not found at ${py}`);
		}
	...
	if (!completed.has("install-reqs")) {
		report("install-reqs");
		const reqFile = node_path.default.join(__dirname, "../../packages/python-backend/requirements.txt");
		if (node_fs.default.existsSync(reqFile)) await runCommand(pip, [
			"install",
			"-r",
			reqFile
		]);
	...
	if (!completed.has("install-ffglitch")) {
		...
		} else report("install-ffglitch", "FFglitch is not available for your platform. Datamoshing effects will be limited.");
	...
	if (!status.pythonOk || !status.ffmpegOk) throw new Error("Installation verification failed. Check the logs for details.");
	report("ready");
	return status;
}
function findProjectRoot() {
	...
	for (const c of candidates) if (node_fs.default.existsSync(node_path.default.join(c, "assets", "bin"))) return c;
	return candidates[0];
}
async function ensureFfmpegBundled() {
	const outDir = bundledAssetDir("ffmpeg");
	node_fs.default.mkdirSync(outDir, { recursive: true });
	const ffmpegOut = node_path.default.join(outDir, `ffmpeg${exeSuffix()}`);
	const ffprobeOut = node_path.default.join(outDir, `ffprobe${exeSuffix()}`);
	if (node_fs.default.existsSync(ffmpegOut) && node_fs.default.existsSync(ffprobeOut)) return;
	const repoRoot = findProjectRoot();
	const repoFfmpegDir = node_path.default.join(repoRoot, "assets", "bin", "ffmpeg-master-latest-win64-gpl", "bin");
	const repoFfmpeg = node_path.default.join(repoFfmpegDir, `ffmpeg${exeSuffix()}`);
	const repoFfprobe = node_path.default.join(repoFfmpegDir, `ffprobe${exeSuffix()}`);
	if (node_fs.default.existsSync(repoFfmpeg) && node_fs.default.existsSync(repoFfprobe)) {
		if (!node_fs.default.existsSync(ffmpegOut)) node_fs.default.copyFileSync(repoFfmpeg, ffmpegOut);
		if (!node_fs.default.existsSync(ffprobeOut)) node_fs.default.copyFileSync(repoFfprobe, ffprobeOut);
		return;
	}
	try {
		const ffmpegStatic = require.resolve("ffmpeg-static");
		const ffprobeStatic = findFfprobeStatic();
		if (ffmpegStatic && node_fs.default.existsSync(ffmpegStatic) && !node_fs.default.existsSync(ffmpegOut)) node_fs.default.copyFileSync(ffmpegStatic, ffmpegOut);
		if (ffprobeStatic && node_fs.default.existsSync(ffprobeStatic) && !node_fs.default.existsSync(ffprobeOut)) node_fs.default.copyFileSync(ffprobeStatic, ffprobeOut);
		if (node_fs.default.existsSync(ffmpegOut) && node_fs.default.existsSync(ffprobeOut)) return;
	} catch {}
	throw new Error(`FFmpeg binaries not found. Searched:\n  - ${repoFfmpeg}\n  - ffmpeg-static npm package\nPlease ensure FFmpeg is bundled in assets/bin/ffmpeg-master-latest-win64-gpl/bin/ or install ffmpeg-static.`);
}
function findFfprobeStatic() {
	try {
		const pkgRoot = node_path.default.dirname(require.resolve("ffprobe-static/package.json"));
		const candidates = [node_path.default.join(pkgRoot, "bin", process.platform, `ffprobe${exeSuffix()}`), node_path.default.join(pkgRoot, `ffprobe${exeSuffix()}`)];
		for (const c of candidates) if (node_fs.default.existsSync(c)) return c;
	} catch {}
}
async function ensureFfglitchBundled() {
	const outDir = bundledAssetDir("ffglitch");
	node_fs.default.mkdirSync(outDir, { recursive: true });
	const ffgacOut = node_path.default.join(outDir, `ffgac${exeSuffix()}`);
	const ffeditOut = node_path.default.join(outDir, `ffedit${exeSuffix()}`);
	if (node_fs.default.existsSync(ffgacOut) && node_fs.default.existsSync(ffeditOut)) return;
	const repoRoot = findProjectRoot();
	const repoFfglitchDir = node_path.default.join(repoRoot, "assets", "bin", "ffglitch-0.10.2-windows-x86_64");
	if (node_fs.default.existsSync(repoFfglitchDir)) {
		const ffgacSrc = node_path.default.join(repoFfglitchDir, `ffgac${exeSuffix()}`);
		const ffeditSrc = node_path.default.join(repoFfglitchDir, `ffedit${exeSuffix()}`);
		if (node_fs.default.existsSync(ffgacSrc) && !node_fs.default.existsSync(ffgacOut)) node_fs.default.copyFileSync(ffgacSrc, ffgacOut);
		if (node_fs.default.existsSync(ffeditSrc) && !node_fs.default.existsSync(ffeditOut)) node_fs.default.copyFileSync(ffeditSrc, ffeditOut);
		if (node_fs.default.existsSync(ffgacOut) && node_fs.default.existsSync(ffeditOut)) return;
	}
	const legacyDir = node_path.default.join(__dirname, "../../assets/bin/ffglitch-0.10.2-windows-x86_64");
	if (node_fs.default.existsSync(legacyDir)) {
		const ffgacSrc = node_path.default.join(legacyDir, `ffgac${exeSuffix()}`);
		const ffeditSrc = node_path.default.join(legacyDir, `ffedit${exeSuffix()}`);
		if (node_fs.default.existsSync(ffgacSrc) && !node_fs.default.existsSync(ffgacOut)) node_fs.default.copyFileSync(ffgacSrc, ffgacOut);
		if (node_fs.default.existsSync(ffeditSrc) && !node_fs.default.existsSync(ffeditOut)) node_fs.default.copyFileSync(ffeditSrc, ffeditOut);
		if (node_fs.default.existsSync(ffgacOut) && node_fs.default.existsSync(ffeditOut)) return;
	}
	throw new Error(`FFglitch binaries not found. Searched:\n  - ${repoFfglitchDir}\nPlease ensure FFglitch is bundled in assets/bin/ffglitch-0.10.2-windows-x86_64/.`);
}
function exeSuffix() {
...
function runCommand(cmd, args) {
	return new Promise((resolve, reject) => {
		const proc = (0, node_child_process.spawn)(cmd, args, { stdio: "inherit" });
		proc.on("close", (code) => {
			if (code === 0) resolve();
			else reject(/* @__PURE__ */ new Error(`Command exited with code ${code}: ${cmd} ${args.join(" ")}`));
		});
...
try {
	...
	electron.app.commandLine.appendSwitch("gpu-sandbox-start-early");
} catch {}
...
installLogger();
var crashCheck = checkCrashMarker();
if (crashCheck.crashed) console.warn("[Logger] Previous session crashed:", crashCheck.info);
var RPC_TOKEN = node_crypto.default.randomBytes(32).toString("hex");
process.env.MOSHDITHER_RPC_TOKEN = RPC_TOKEN;
var __dirname$1 = (0, node_path.dirname)((0, node_url.fileURLToPath)(require("url").pathToFileURL(__filename).href));
var projectRoot = node_path.default.resolve(__dirname$1, "../../../");
var binaries = {
	python: process.platform === "win32" ? "python" : "python3",
	moshCli: node_path.default.join(projectRoot, "packages/python-backend/mosh_cli.py"),
	ditherCli: node_path.default.join(projectRoot, "references/dither_pie/dither_cli.py"),
	ffmpeg: node_path.default.join(projectRoot, "assets/bin/ffmpeg-master-latest-win64-gpl/bin/ffmpeg.exe"),
	ffprobe: node_path.default.join(projectRoot, "assets/bin/ffmpeg-master-latest-win64-gpl/bin/ffprobe.exe"),
	ffgac: node_path.default.join(projectRoot, "assets/bin/ffglitch-0.10.2-windows-x86_64/ffgac.exe"),
	ffedit: node_path.default.join(projectRoot, "assets/bin/ffglitch-0.10.2-windows-x86_64/ffedit.exe")
};
...
var mosher = new MosherAdapter(binaries.python, binaries.moshCli, binaries.ffedit, binaries.ffmpeg);
var ditherer = new DitherAdapter(binaries.python, binaries.ditherCli);
function runFfmpeg(args) {
	return new Promise((resolve, reject) => {
		console.log(`Spawning ffmpeg: ${binaries.ffmpeg} ${args.join(" ")}`);
		const proc = (0, node_child_process.spawn)(binaries.ffmpeg, args);
		let stderr = "";
		proc.stderr.on("data", (data) => {
			stderr += data.toString();
		});
		proc.on("close", (code) => {
			if (code === 0) resolve();
			else reject(/* @__PURE__ */ new Error(`ffmpeg exited with code ${code}. Error: ${stderr}`));
		});
	});
}
function readMediaMetadata(filePath) {
	return new Promise((resolve) => {
		const proc = (0, node_child_process.spawn)(binaries.ffprobe, [
			"-v",
			"quiet",
			"-print_format",
			"json",
...
var distPath = process.env.DIST || node_path.default.join(__dirname$1, "../dist");
process.env.VITE_PUBLIC = electron.app.isPackaged ? distPath : node_path.default.join(distPath, "../public");
var win;
...
function createWindow() {
	...
	win.webContents.on("will-navigate", (event, url) => {
		if (!(VITE_DEV_SERVER_URL ? url.startsWith(VITE_DEV_SERVER_URL) : false) && url !== win?.webContents.getURL()) {
	...
	else win.loadFile(node_path.default.join(distPath, "index.html"));
}
...
function validateMediaPath(rawPath) {
	...
	const normalized = node_path.default.normalize(rawPath);
	if (!node_path.default.isAbsolute(normalized)) {
		console.error("Blocked relative media path:", rawPath);
		return null;
	}
	return normalized;
}
function getPathFromMediaUrl(mediaUrl) {
	let rawPath;
	try {
		const url = new URL(decodeURIComponent(mediaUrl));
		if (url.host) if (/^[a-zA-Z]$/.test(url.host)) rawPath = url.host + ":" + url.pathname;
		else {
...
*/
function verifyPreloadIntegrity() {
	const preloadPath = node_path.default.join(__dirname$1, "preload.js");
	const hashPath = node_path.default.join(__dirname$1, "preload.hash");
	if (!node_fs.default.existsSync(hashPath)) {
		console.warn("[SRI] No preload.hash found. Skipping integrity check.");
		return;
	}
	const expected = node_fs.default.readFileSync(hashPath, "utf-8").trim();
	const actual = node_crypto.default.createHash("sha256").update(node_fs.default.readFileSync(preloadPath)).digest("hex");
	if (expected !== actual) throw new Error(`Preload integrity mismatch: expected ${expected}, got ${actual}`);
...
function validateIpcSender(event) {
	...
	const isLocalhost = !!process.env["VITE_DEV_SERVER_URL"] && (senderUrl.startsWith("http://localhost:") || senderUrl.startsWith("http://127.0.0.1:"));
	if (!(senderUrl.startsWith("file://") || senderUrl.startsWith("media://") || isLocalhost || (VITE_DEV_SERVER_URL ? senderUrl.startsWith(VITE_DEV_SERVER_URL) : false))) throw new Error(`Unauthorized IPC sender: ${senderUrl}`);
}
startPythonBackend().catch((err) => {
	console.error("[Main] Failed to start Python backend:", err);
});
electron.app.whenReady().then(() => {
	const projectRoot = node_path.default.resolve(__dirname$1, "../../..");
	const defaultOutputDir = node_path.default.join(projectRoot, "outputs");
	if (!node_fs.default.existsSync(defaultOutputDir)) node_fs.default.mkdirSync(defaultOutputDir, { recursive: true });
	addStdoutListener((text) => {
		...
		if (text.includes("SAM3_DOWNLOAD_COMPLETE") && win && !win.isDestroyed()) win.webContents.send("sam3:download-progress", {
			model: "sam3",
			progress: 100,
			status: "success"
		});
	});
	function createRpcSessionToken() {
		...
		return `${header}.${payload}.${node_crypto.default.createHmac("sha256", RPC_TOKEN).update(`${header}.${payload}`).digest("base64url")}`;
	}
	electron.ipcMain.handle("get-rpc-token", (event) => {
		validateIpcSender(event);
		return createRpcSessionToken();
	});
	electron.ipcMain.handle("get-default-output-dir", (event) => {
		validateIpcSender(event);
		return defaultOutputDir.replace(/\\/g, "/");
	});
	electron.ipcMain.handle("dialog:selectOutputDir", async (event) => {
		validateIpcSender(event);
		const { canceled, filePaths } = await electron.dialog.showOpenDialog({ properties: ["openDirectory", "createDirectory"] });
		if (!canceled && filePaths.length > 0) return filePaths[0].replace(/\\/g, "/");
		return null;
	});
	electron.ipcMain.handle("dialog:selectSyncFolder", async (event) => {
		validateIpcSender(event);
		const { canceled, filePaths } = await electron.dialog.showOpenDialog({ properties: ["openDirectory", "createDirectory"] });
		if (!canceled && filePaths.length > 0) return filePaths[0].replace(/\\/g, "/");
		return null;
	});
	electron.ipcMain.handle("sync:write-project", async (event, folderPath, projectId, json) => {
		validateIpcSender(event);
		try {
			const filePath = node_path.default.join(folderPath, `${projectId}.moshdither`);
			node_fs.default.writeFileSync(filePath, json, "utf-8");
			return true;
		} catch {
			return false;
		}
	});
	electron.ipcMain.handle("sync:list-projects", async (event, folderPath) => {
		validateIpcSender(event);
		try {
			return node_fs.default.readdirSync(folderPath).filter((f) => f.endsWith(".moshdither")).map((f) => {
				const filePath = node_path.default.join(folderPath, f);
				...
				try {
					name = JSON.parse(content).project?.name || name;
				} catch {}
				return {
	...
	electron.ipcMain.handle("git:lfs-status", async (event) => {
		validateIpcSender(event);
		try {
			...
			return {
				installed: true,
				initialized: true,
				trackedPatterns: execSync("git lfs track", {
					encoding: "utf-8",
					cwd: projectRoot
				}).split("\n").filter(Boolean)
			};
	...
	electron.ipcMain.handle("git:init-lfs", async (event, projectPath) => {
		validateIpcSender(event);
		try {
			const { execSync } = await import("node:child_process");
			execSync("git init", {
	...
	electron.ipcMain.handle("git:write-attributes", async (event, projectPath, content) => {
		validateIpcSender(event);
		try {
			const attributesPath = node_path.default.join(projectPath, ".gitattributes");
			node_fs.default.writeFileSync(attributesPath, content, "utf-8");
			return true;
		} catch {
			return false;
		}
	});
	electron.ipcMain.handle("sam3:load-model", async (event) => {
		validateIpcSender(event);
		try {
			return {
				ok: true,
				loaded: (await pythonRpcCall("sam3_list_models", {})).models?.sam3 ?? false
			};
		} catch (err) {
	...
	electron.ipcMain.handle("sam3:preload-model", async (event) => {
		validateIpcSender(event);
		try {
			const result = await pythonRpcCall("sam3_load_model", {});
	...
	electron.ipcMain.handle("sam3:predict-batch", async (event, payload) => {
		validateIpcSender(event);
		const { imagePath, positivePoints, negativePoints } = payload;
		try {
			const filePath = imagePath.startsWith("media://") ? getPathFromMediaUrl(imagePath) : imagePath;
			if (!filePath || !node_fs.default.existsSync(filePath)) throw new Error(`Image file not found: ${imagePath}`);
			...
			if (result.status !== "success" || !result.mask) throw new Error(result.error || "SAM 3 prediction failed");
			return {
				ok: true,
	...
	electron.ipcMain.handle("sam3:predict-box", async (event, payload) => {
		validateIpcSender(event);
		const { imagePath, x1, y1, x2, y2 } = payload;
		try {
			const filePath = imagePath.startsWith("media://") ? getPathFromMediaUrl(imagePath) : imagePath;
			if (!filePath || !node_fs.default.existsSync(filePath)) throw new Error(`Image file not found: ${imagePath}`);
			...
			if (result.status !== "success" || !result.mask) throw new Error(result.error || "SAM 3 box prediction failed");
			return {
				ok: true,
				maskBase64: result.mask,
				width: result.width,
	...
	electron.ipcMain.handle("sam3:hover-preview", async (event, payload) => {
		validateIpcSender(event);
		const { imagePath, point } = payload;
		try {
			const filePath = imagePath.startsWith("media://") ? getPathFromMediaUrl(imagePath) : imagePath;
			if (!filePath || !node_fs.default.existsSync(filePath)) throw new Error(`Image file not found: ${imagePath}`);
			...
			if (result.status !== "success" || !result.mask) throw new Error(result.error || "SAM 3 hover preview failed");
			return {
				ok: true,
				maskBase64: result.mask,
				width: result.width,
	...
	electron.ipcMain.handle("sam3:predict-text", async (event, payload) => {
		validateIpcSender(event);
		const { imagePath, textPrompt } = payload;
		try {
			const filePath = imagePath.startsWith("media://") ? getPathFromMediaUrl(imagePath) : imagePath;
			if (!filePath || !node_fs.default.existsSync(filePath)) throw new Error(`Image file not found: ${imagePath}`);
			...
			if (result.status !== "success" || !result.mask) throw new Error(result.error || "SAM 3 text prediction failed");
			return {
				ok: true,
				maskBase64: result.mask,
				width: result.width,
	...
	electron.ipcMain.handle("sam3:predict-point-pro", async (event, payload) => {
		validateIpcSender(event);
		const { imagePath, point } = payload;
		try {
			const filePath = imagePath.startsWith("media://") ? getPathFromMediaUrl(imagePath) : imagePath;
			...
			if (result.status !== "success" || !result.mask) throw new Error(result.error || "SAM 3 PRO point prediction failed");
			return {
				ok: true,
				maskBase64: result.mask,
				width: result.width,
	...
	electron.ipcMain.handle("sam3:predict-text-pro", async (event, payload) => {
		validateIpcSender(event);
		const { imagePath, textPrompt } = payload;
		try {
			const filePath = imagePath.startsWith("media://") ? getPathFromMediaUrl(imagePath) : imagePath;
			...
			if (result.status !== "success" || !result.mask) throw new Error(result.error || "SAM 3 PRO text prediction failed");
			return {
				ok: true,
				maskBase64: result.mask,
				width: result.width,
	...
	electron.ipcMain.handle("sam3:predict-groundingdino", async (event, payload) => {
		validateIpcSender(event);
		const { imagePath, textPrompt, threshold } = payload;
		try {
			const filePath = imagePath.startsWith("media://") ? getPathFromMediaUrl(imagePath) : imagePath;
			if (!filePath || !node_fs.default.existsSync(filePath)) throw new Error(`Image file not found: ${imagePath}`);
			...
			if (result.status !== "success" || !result.mask) throw new Error(result.error || "GroundingDINO prediction failed");
			return {
				ok: true,
				maskBase64: result.mask,
				width: result.width,
	...
	electron.ipcMain.handle("sam3:list-models", async (event) => {
		validateIpcSender(event);
		try {
			return await pythonRpcCall("sam3_list_models", {});
		} catch (err) {
	...
	electron.ipcMain.handle("sam3:download-model", async (event) => {
		validateIpcSender(event);
		try {
			return { ok: (await pythonRpcCall("sam3_download_model", {})).status === "success" };
		} catch (err) {
			const msg = err instanceof Error ? err.message : String(err);
	...
	electron.ipcMain.handle("sam3:model-status", async (event) => {
		validateIpcSender(event);
		try {
			const result = await pythonRpcCall("sam3_model_status", {});
			return {
				active_downloads: result.status === "downloading" ? ["sam3"] : [],
	...
	electron.ipcMain.handle("sam3:unload-model", async () => {
		try {
	...
	electron.ipcMain.handle("sam3:remove-background", async (event, payload) => {
		validateIpcSender(event);
		const { imagePath, model, alphaMatting } = payload;
		try {
			const filePath = imagePath.startsWith("media://") ? getPathFromMediaUrl(imagePath) : imagePath;
			if (!filePath || !node_fs.default.existsSync(filePath)) throw new Error(`Image file not found: ${imagePath}`);
			...
			if (result.status !== "success" || !result.mask) throw new Error(result.error || "Background removal failed");
			return {
				ok: true,
				maskBase64: result.mask,
				width: result.width,
	...
	electron.ipcMain.handle("sam3:remove-background-batch", async (event, payload) => {
		validateIpcSender(event);
		const { imagePaths, model, alphaMatting } = payload;
		try {
			return {
				ok: true,
				results: await pythonRpcCall("sam3_remove_background_batch", {
					image_paths: imagePaths.map((p) => p.startsWith("media://") ? getPathFromMediaUrl(p) : p),
					model: model || "u2net",
					alpha_matting: alphaMatting ?? false
				})
			};
	...
	electron.ipcMain.handle("mask:post-process", async (event) => {
		validateIpcSender(event);
		return {
			ok: false,
			error: "mask:post-process should be handled in renderer WebWorker. Use maskPostProcessing.ts."
		};
	});
	electron.ipcMain.handle("mask:flood-fill", async (event) => {
		validateIpcSender(event);
		return {
			ok: false,
	...
	electron.ipcMain.handle("fonts:list", async (event) => {
		validateIpcSender(event);
		try {
			const fonts = await (0, import_dist.default)({ extensions: [
				"ttf",
				"otf",
				"ttc"
			] });
	...
	electron.protocol.handle("media", async (request) => {
		try {
			...
			const response = await electron.net.fetch((0, node_url.pathToFileURL)(decodedPath).toString());
	...
	let webglExportResolve = null;
	electron.ipcMain.handle("main:save-webgl-blob", async (event, { data, ext }) => {
		validateIpcSender(event);
		const tmpPath = node_path.default.join(node_os.default.tmpdir(), `webgl_export_${Date.now()}${ext}`);
		node_fs.default.writeFileSync(tmpPath, Buffer.from(data));
	...
	async function captureWebGLFromRenderer(inputUrl, _effects, duration, fps) {
		return new Promise((resolve, reject) => {
			if (!win) {
				reject(/* @__PURE__ */ new Error("No renderer window available for WebGL capture"));
				return;
			}
			...
			setTimeout(() => {
				if (webglExportResolve) {
					webglExportResolve = null;
					reject(/* @__PURE__ */ new Error("WebGL export timed out after 120s"));
				}
			}, 12e4);
	...
	electron.ipcMain.handle("dialog:openMedia", async (event) => {
		validateIpcSender(event);
		...
		if (!canceled && filePaths.length > 0) return `media://${filePaths[0].replace(/\\/g, "/")}`;
		return null;
	});
	electron.ipcMain.handle("dialog:openMediaMultiple", async (event) => {
		validateIpcSender(event);
		const { canceled, filePaths } = await electron.dialog.showOpenDialog({
		...
		if (!canceled && filePaths.length > 0) return filePaths.map((p) => `media://${p.replace(/\\/g, "/")}`);
		return [];
	});
	electron.ipcMain.handle("safe-storage:read", async (event, key) => {
		validateIpcSender(event);
		if (!electron.safeStorage.isEncryptionAvailable()) return null;
		const filePath = node_path.default.join(electron.app.getPath("userData"), "safe-storage", `${key}.bin`);
		try {
			const encrypted = node_fs.default.readFileSync(filePath);
			return electron.safeStorage.decryptString(encrypted);
		} catch {
			return null;
		}
	});
	electron.ipcMain.handle("safe-storage:write", async (event, key, value) => {
		validateIpcSender(event);
		if (!electron.safeStorage.isEncryptionAvailable()) return;
		const dir = node_path.default.join(electron.app.getPath("userData"), "safe-storage");
		if (!node_fs.default.existsSync(dir)) node_fs.default.mkdirSync(dir, { recursive: true });
		const filePath = node_path.default.join(dir, `${key}.bin`);
		const encrypted = electron.safeStorage.encryptString(value);
		node_fs.default.writeFileSync(filePath, encrypted);
	});
	electron.ipcMain.handle("safe-storage:delete", async (event, key) => {
		validateIpcSender(event);
		const filePath = node_path.default.join(electron.app.getPath("userData"), "safe-storage", `${key}.bin`);
		try {
	...
	electron.ipcMain.handle("env:status", async (event) => {
		validateIpcSender(event);
		return getEnvironmentStatus();
	});
	electron.ipcMain.handle("env:install-local", async (event) => {
		validateIpcSender(event);
		try {
			return {
				ok: true,
				status: await installLocalEnvironment((progress) => {
	...
	electron.ipcMain.handle("env:set-mode", async (event, mode) => {
		validateIpcSender(event);
		saveEnvConfig(mode);
		return { ok: true };
	});
	const PROXY_DIR = node_path.default.join(node_os.default.homedir(), ".moshdither", "proxies");
	function getProxyPath(originalPath) {
		const hash = node_crypto.default.createHash("sha256").update(originalPath).digest("hex").slice(0, 32);
		const ext = node_path.default.extname(originalPath);
		return node_path.default.join(PROXY_DIR, `${hash}_proxy${ext}`);
	}
	electron.ipcMain.handle("proxy:has", async (event, mediaUrl) => {
		validateIpcSender(event);
		const originalPath = getPathFromMediaUrl(mediaUrl);
		if (!originalPath) return false;
		const proxyPath = getProxyPath(originalPath);
		return node_fs.default.existsSync(proxyPath);
	});
	electron.ipcMain.handle("proxy:generate", async (event, mediaUrl) => {
		validateIpcSender(event);
		const originalPath = getPathFromMediaUrl(mediaUrl);
		if (!originalPath) throw new Error("Invalid media URL");
		const proxyPath = getProxyPath(originalPath);
		if (node_fs.default.existsSync(proxyPath)) return `media://${proxyPath.replace(/\\/g, "/")}`;
		if (!node_fs.default.existsSync(PROXY_DIR)) node_fs.default.mkdirSync(PROXY_DIR, { recursive: true });
		return new Promise((resolve, reject) => {
			const args = [
				"-i",
				originalPath,
			...
			proc.stderr.on("data", () => {});
			proc.on("close", (code) => {
				if (code === 0) resolve(`media://${proxyPath.replace(/\\/g, "/")}`);
				else reject(/* @__PURE__ */ new Error(`ffmpeg proxy generation failed (code ${code})`));
			});
		});
	});
	electron.ipcMain.handle("proxy:cleanup", async (event) => {
		validateIpcSender(event);
		if (!node_fs.default.existsSync(PROXY_DIR)) return;
		const now = Date.now();
		const maxAge = 720 * 60 * 60 * 1e3;
		for (const file of node_fs.default.readdirSync(PROXY_DIR)) {
			const filePath = node_path.default.join(PROXY_DIR, file);
			if (now - node_fs.default.statSync(filePath).mtime.getTime() > maxAge) node_fs.default.unlinkSync(filePath);
		}
	});
	electron.ipcMain.handle("cache:get-frame", async (event, key) => {
		validateIpcSender(event);
		return getCachedFrame(key);
	});
	electron.ipcMain.handle("cache:set-frame", async (event, key, data) => {
		validateIpcSender(event);
	...
	electron.ipcMain.handle("cache:stats", async (event) => {
		validateIpcSender(event);
		return getCacheStats();
	});
	electron.ipcMain.handle("metadata:read", async (event, mediaUrl) => {
		validateIpcSender(event);
	...
	electron.ipcMain.handle("render:pipeline", async (event, inputUrl, effects, outputDirectory, exportFormat, exportFps, useProxy, watermarkSettings) => {
		validateIpcSender(event);
		const tempFiles = [];
		try {
			let currentPath = getPathFromMediaUrl(inputUrl);
			if (!currentPath) throw new Error("Invalid or unsafe input URL provided.");
			if (useProxy) {
				const proxyPath = getProxyPath(currentPath);
				if (node_fs.default.existsSync(proxyPath)) currentPath = proxyPath;
			}
			const originalExt = node_path.default.extname(currentPath);
			...
			console.log(`Starting Render Pipeline on: ${currentPath} with ${effects.length} effects. Target format: ${exportFormat || "same"} (${targetExt})`);
			const totalSteps = effects.filter((fx) => fx.enabled).length + 2;
			...
			if (effects.some((fx) => fx.enabled && webglTypes.has(fx.type)) && win) {
				const webglInputUrl = currentPath.startsWith("media://") ? currentPath : `media://${currentPath.replace(/\\/g, "/")}`;
				const duration = effects.find((fx) => typeof fx.endTime === "number")?.endTime || 10;
				console.log(`Delegating WebGL effects to renderer for ${duration}s at ${exportFps || 30}fps`);
				currentPath = await captureWebGLFromRenderer(webglInputUrl, effects.filter((fx) => fx.enabled && webglTypes.has(fx.type)), duration, exportFps || 30);
			...
			for (const fx of effects) {
				...
				if (fx.type === "datamosh") {
					const tempOut = node_path.default.join(node_os.default.tmpdir(), `mosh_step_${Date.now()}${originalExt}`);
					const params = { ...fx.params };
					params.mode = params.mode || "classic";
					if (params.motionUrl) {
						const motionPath = getPathFromMediaUrl(params.motionUrl);
						if (!motionPath) throw new Error("Invalid or unsafe motion URL provided.");
					...
					reportProgress(`Applied dither (${fx.params.ditherMode || "atkinson"})`);
					...
					currentPath = await ditherer.applyDither(currentPath, {
						...
						angleK: fx.params.angleK || 45
					});
					reportProgress("Applied halftone");
				}
			}
			const currentExt = node_path.default.extname(currentPath).toLowerCase();
			const targetExtLower = targetExt.toLowerCase();
			if (currentExt !== targetExtLower) {
				console.log(`Converting output from ${currentExt} to ${targetExtLower} with target FPS: ${exportFps || 30}`);
				const conversionTempPath = node_path.default.join(node_os.default.tmpdir(), `conv_step_${Date.now()}${targetExtLower}`);
				const videoExts = [
					".mp4",
					".avi",
					".mov",
				...
				if (isCurrentImage && isTargetVideo) {
					...
					if (effects && effects.length > 0) {
						...
						if (firstActive && typeof firstActive.endTime === "number" && typeof firstActive.startTime === "number") {
							const diff = firstActive.endTime - firstActive.startTime;
							if (diff > 0) loopDuration = diff;
						}
					}
					if (targetExtLower === ".mp4") args = [
						"-loop",
						"1",
						"-framerate",
						fps.toString(),
					...
					else if (targetExtLower === ".gif") args = [
						"-loop",
						"1",
						"-framerate",
						fps.toString(),
				...
				} else if (isCurrentVideo && targetExtLower === ".gif") args = [
					"-i",
					currentPath,
					"-map_metadata",
					"0",
				...
				else if (isCurrentVideo && isTargetImage) args = [
					"-ss",
					"0",
					"-i",
					currentPath,
				...
				else if (targetExtLower === ".mp4") args = [
					"-i",
					currentPath,
					"-map_metadata",
					"0",
				...
				if (args.length > 0) {
					const { args: watermarkedArgs } = buildWatermarkArgs(args, watermarkSettings || {
						enabled: false,
						type: "text",
						text: "",
						imagePath: null,
			...
			const originalPath = decodeURIComponent(inputUrl.replace("media://", ""));
			const ext = node_path.default.extname(originalPath);
			const outputName = node_path.default.basename(originalPath, ext) + "_rendered" + targetExt;
			...
			const finalOutputPath = node_path.default.join(targetDir, outputName);
			node_fs.default.copyFileSync(currentPath, finalOutputPath);
			reportProgress(`Saved to ${finalOutputPath}`);
			const normalized = finalOutputPath.replace(/\\/g, "/");
			for (const tmp of tempFiles) try {
				if (node_fs.default.existsSync(tmp) && tmp !== finalOutputPath) node_fs.default.unlinkSync(tmp);
			} catch {}
			return `media://${normalized}`;
		} catch (err) {
			console.error("Render pipeline error:", err);
			for (const tmp of tempFiles) try {
				if (node_fs.default.existsSync(tmp)) node_fs.default.unlinkSync(tmp);
			} catch {}
			return null;
		}
	});
	electron.ipcMain.on("renderer:log", (event, payload) => {
		validateIpcSender(event);
		writeRendererLog(payload.level, payload.message, payload.details);
	});
	const AUTOSAVE_DIR = node_path.default.join(node_os.default.homedir(), ".moshdither", "autosave");
	const AUTOSAVE_PATH = node_path.default.join(AUTOSAVE_DIR, "project.json");
	const CRASH_MARKER_PATH = node_path.default.join(node_os.default.homedir(), ".moshdither", "crash.marker");
	electron.ipcMain.handle("autosave:write", (event, data) => {
		validateIpcSender(event);
		if (Buffer.byteLength(data, "utf-8") > 5 * 1024 * 1024) {
			console.error("[AutoSave] Payload exceeds 5 MB limit; rejecting write.");
			return false;
		}
		if (!node_fs.default.existsSync(AUTOSAVE_DIR)) node_fs.default.mkdirSync(AUTOSAVE_DIR, { recursive: true });
		node_fs.default.writeFileSync(AUTOSAVE_PATH, data, "utf-8");
		return true;
	});
	electron.ipcMain.handle("autosave:read", (event) => {
		validateIpcSender(event);
		if (node_fs.default.existsSync(AUTOSAVE_PATH)) return node_fs.default.readFileSync(AUTOSAVE_PATH, "utf-8");
		return null;
	});
	electron.ipcMain.handle("autosave:clear", (event) => {
		validateIpcSender(event);
		if (node_fs.default.existsSync(AUTOSAVE_PATH)) node_fs.default.unlinkSync(AUTOSAVE_PATH);
		return true;
	});
	electron.ipcMain.handle("crash:check", (event) => {
		validateIpcSender(event);
		return node_fs.default.existsSync(CRASH_MARKER_PATH);
	});
	electron.ipcMain.handle("crash:dismiss", (event) => {
		validateIpcSender(event);
		if (node_fs.default.existsSync(CRASH_MARKER_PATH)) node_fs.default.unlinkSync(CRASH_MARKER_PATH);
		return true;
	});
	process.on("uncaughtException", () => {
		try {
			node_fs.default.mkdirSync(node_path.default.dirname(CRASH_MARKER_PATH), { recursive: true });
			node_fs.default.writeFileSync(CRASH_MARKER_PATH, (/* @__PURE__ */ new Date()).toISOString(), "utf-8");
		} catch {}
	});
	try {
	...
	const template = [
		...
		}],
		{
			...
			submenu: [
				...
				{
					label: "Documentation",
					click: () => electron.shell.openExternal("https://github.com/richk/MoshDither-Studio")
				},
				{ type: "separator" },
				{
	...
	if (process.platform === "darwin" && win) {
		const { TouchBarButton, TouchBarSpacer } = electron.TouchBar;
		const touchBar = new electron.TouchBar({ items: [
			new TouchBarButton({
				label: "Play/Pause",
	...
	if (process.platform === "win32") electron.app.setJumpList([{
		type: "custom",
...
//#endregion
exports.readMediaMetadata = readMediaMetadata;
