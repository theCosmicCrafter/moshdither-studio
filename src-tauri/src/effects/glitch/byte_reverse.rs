...
impl ByteReverse {
    pub fn new(chunk_size: u32) -> Self { Self { chunk_size: chunk_size.max(2) } }
}
...
impl Effect for ByteReverse {
    ...
    fn process_frame(&self, input: &Frame, _m: Option<&Mask>, params: &ParameterValues) -> Result<Frame> {
        let chunk_size = params.get("chunk_size").and_then(|v| v.as_u64()).unwrap_or(self.chunk_size as u64) as usize;
...
mod tests {
    ...
    fn test_byte_reverse() {
        ...
        let f = Frame { width: 3, height: 1, data: d };
