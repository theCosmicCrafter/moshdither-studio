...
impl Posterize {
    pub fn new(bits: u8) -> Self { Self { bits: bits.clamp(1, 8) } }
}
...
impl Effect for Posterize {
    ...
    fn process_frame(&self, input: &Frame, _m: Option<&Mask>, params: &ParameterValues) -> Result<Frame> {
        let bits = params.get("bits").and_then(|v| v.as_u64()).unwrap_or(self.bits as u64) as u8;
        let shift = 8 - bits;
        let mut data = input.data.clone();
        for chunk in data.chunks_exact_mut(4) {
            for c in &mut chunk[..3] { *c = (*c >> shift) << shift; }
        }
