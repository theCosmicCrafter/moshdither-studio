...
impl SaltPepperNoise {
    pub fn new(density: f32) -> Self { Self { density: density.clamp(0.0, 1.0) } }
}

impl Default for SaltPepperNoise {
    fn default() -> Self { Self::new(0.05) }
}
...
impl Effect for SaltPepperNoise {
    ...
    fn process_frame(&self, input: &Frame, _m: Option<&Mask>, params: &ParameterValues) -> Result<Frame> {
        let density = params.get("density").and_then(|v| v.as_f64()).unwrap_or(self.density as f64) as f32;
        let time = params.get("time").and_then(|v| v.as_f64()).unwrap_or(0.0) as u32;
        let mut data = input.data.clone();
        let count = (data.len() as f32 / 4.0 * density) as usize;
        for i in 0..count {
            let seed = (i as u32).wrapping_add(time.wrapping_mul(127)).wrapping_mul(374761393).wrapping_add(668265263);
...
mod tests {
    ...
    fn test_salt_pepper() {
        ...
        for i in 0..r.data.len()/4 {
            if r.data[i*4] != 128 { changed = true; break; }
        }
        assert!(changed);
    }
}