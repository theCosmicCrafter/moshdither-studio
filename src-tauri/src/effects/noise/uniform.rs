...
impl UniformNoise {
    pub fn new(range: u8) -> Self { Self { range } }
}
...
impl Effect for UniformNoise {
    ...
    fn process_frame(&self, input: &Frame, _m: Option<&Mask>, params: &ParameterValues) -> Result<Frame> {
        let range = params.get("range").and_then(|v| v.as_u64()).unwrap_or(self.range as u64) as i16;
        let time = params.get("time").and_then(|v| v.as_f64()).unwrap_or(0.0) as u32;
        let mut data = input.data.clone();
        for (i, chunk) in data.chunks_exact_mut(4).enumerate() {
            let seed = (i as u32).wrapping_add(time.wrapping_mul(101)).wrapping_mul(374761393u32);
