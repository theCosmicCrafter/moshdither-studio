...
impl Ghosting {
    pub fn new(intensity: f32, offset: u32) -> Self {
        Self { intensity: intensity.clamp(0.0, 1.0), offset }
    }
}

impl Default for Ghosting {
    fn default() -> Self { Self::new(0.3, 4) }
}
...
impl Effect for Ghosting {
    ...
    fn process_frame(&self, input: &Frame, _m: Option<&Mask>, params: &ParameterValues) -> Result<Frame> {
        ...
        let offset = params.get("offset").and_then(|v| v.as_u64()).unwrap_or(self.offset as u64) as usize;
        ...
        for y in 0..h {
            for x in offset..w {
                ...
                for c in 0..3 {
                    let v = data[idx + c] as f32 * (1.0 - intensity) + input.data[ghost_idx + c] as f32 * intensity;
...
mod tests {
    ...
    fn test_ghosting() {
        ...
        let f = Frame { width: 8, height: 4, data: d };
