...
## 2026-07-19 (later) — Custom LUT review / UX hardening

### Goal

Make the custom LUT workflow work for all users and harden the code surfaced by the `/review` pass and the app-start smoke test.

### What changed

- **Secure custom LUT loading**
  - New `prepare_custom_lut` Tauri command in `src-tauri/src/commands.rs`:
    - Validates the selected path through `path_guard::validate_io_path`.
    - Enforces `.png` or `.cube` extension and a 50 MB size limit.
    - Copies the LUT into `$TEMP/moshdither-studio/luts/` so the WebView asset protocol can preview it without broadening the scope.
  - `src/components/LUTPanel/index.tsx` calls `prepare_custom_lut` before `loadCustomLUT`, ensuring the preview URL is inside the allowed asset scope.

- **Rust LUT path validation**
  - `src-tauri/src/effects/color/lut_grading.rs` `locate_lut_file()` now validates absolute paths with `validate_io_path()` before reading.

- **.cube robustness**
  - Added `MAX_CUBE_SIZE = 256` guard in Rust and `parseLut.ts` to prevent runaway allocation.
  - `apply_cube()` clamps sampled LUT values to `[0, 1]` before blending.

- **Documentation / UX**
  - `README.md` gained a **LUT setup** section with bundled-preset folders and custom-LUT loading steps.
  - `LUTPanel` now shows inline help text explaining supported formats and preset locations.
...
- `cargo test --lib` 438/438 PASS
- `cargo fmt -- --check` PASS
- `cargo audit` PASS (0 vulns, 18 unmaintained warnings)
- `npm audit` PASS
- `npm run lint` PASS (0 warnings)
- `npx tsc --noEmit` PASS
- `npm run test` 1024/1024 PASS
- `cargo run --bin mosh-verify -- verify-all` 98/98 PASS
- `../tools/scan-gate.ps1` CLEAN
