...
| 18  | Release signing verification        | **Out of scope** | —                                                             | Requires platform-specific signature APIs (Authenticode / codesign) and CI infrastructure. Not a coding-session task   |
...
| 7   | Multi-track layering           | **Out of scope** | —                                                                                             | Architectural — requires compositor rewrite for N video/image tracks with blend modes and track-level effects. Not a coding-session task.                |
| 8   | Time remapping / speed ramps   | **Out of scope** | —                                                                                             | Architectural — requires non-linear time mapping, frame interpolation, and resampling pipeline. Not a coding-session task.                               |
| 9   | Beat detection / auto-keyframe | **Done**         | `utils/beatDetection.ts`                                                                      | Spectral-flux onset detector via Web Audio API. Estimates BPM. Auto-keyframe generation from beat timestamps is a UI task, not ML.                       |
...
| 5   | MIDI control             | **Out of scope** | `utils/midiControl.ts`, `molecules/MIDIManager.tsx`, `hooks/useMIDI.ts` | ~~MIDI learn, mapping registry. Wired to `StudioContext` — CC messages update effect params via `setActiveEffects`~~ — Live hardware controller integration. VJ/performance territory, not post-production. Code retained but not a priority.                                   |
| 6   | OSC / DMX                | **Out of scope** | `utils/oscDMX.ts`                                                       | ~~OSC parser (`parseOSC`). Chrome UDP listener stub. DMX stub.~~ — Live show control protocols (TouchDesigner, Ableton, lighting). `startOSCListener` uses Chrome sockets API which does not work in Electron renderer with contextIsolation. Code retained but not a priority. |
...
| 9   | Watermark / overlay   | **Done** | `src/utils/watermark.ts`, `electron/main.ts`, `components/layout/PropertiesPanel.tsx`                                    | Text (drawtext) and image (overlay) watermarks. Position (5 presets), opacity, size/scale, color picker. Settings persisted to localStorage. Applied during FFmpeg final conversion in render pipeline. **Completed by Agent 1.** |
...
| 3   | Browser tests        | **Done** | `WebGLCanvas.browser.test.tsx` | `@vitest/browser@3.2.6` + Playwright Chromium. 3 tests passing: canvas mounts, WebGL2 context creation, cleanup on unmount. Run via `npm run test:browser`                                       |
...
| 3   | GPU capability detection | **Partial** | `utils/webgl/gpuDetector.ts`                                   | WebGL2 check exists. Canvas2D fallback is a renderer pipeline change, not a feature add. Not ML-related                                          |
...
| #   | Item                      | Status           | Verified                                                                                                           | Notes                                                                                                                                                                                                                                      |
| --- | ------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | Plugin system             | **Out of scope** | `utils/pluginSystem.ts`, `molecules/PluginManager.tsx`                                                             | Requires sandboxed iframe or QuickJS VM for third-party JS execution. Security architecture, not a coding-session task.                                                                                                                    |
| 2   | ~~NDI / Syphon / Spout~~  | **Removed**      | —                                                                                                                  | ~~Not relevant for a post-production tool. This is live VJ/performance software territory. Stub file `ndiOutput.ts` removed.~~                                                                                                             |
...
| 5   | Real-time collaboration   | **Out of scope** | `electron/collaborationServer.ts`, `src/utils/collaboration.ts`, `src/components/organisms/CollaborationPanel.tsx` | ~~WebSocket room server. Effect stack sync via ops. Presence tracking with avatars.~~ — Useful for team review but not core post-production functionality. Code retained but not a priority.                                               |
...
| 2   | Auto-keyframe (music sync)     | **Done**     | `utils/beatDetection.ts`, `utils/beatKeyframeGenerator.ts`, `components/layout/PropertiesPanel.tsx` | Beat detection → keyframe generation. 4 modes: Pulse (spike+decay), Toggle (alternate), Ramp (cycle), Decay (peak+fade). UI in PropertiesPanel with param/mode/min/max selection |
| 3   | Object/face tracking for masks | **Done**     | `hooks/useSAM3.ts`, `components/canvas/WebGLCanvas.tsx`, `components/organisms/Viewport.tsx`        | SAM 3 (onnx-community/sam3-tracker-ONNX) click-to-segment masking. Lazy-loaded, cached in app data. Effect mask type "sam" with base64 PNG storage                               |
| 4   | Smart export settings          | **Done**     | `utils/smartExport.ts`                                                                              | See Section 7.8 — same feature. Heuristic-based recommendation engine                                                                                                            |
| 5   | AI style transfer              | **Needs ML** | —                                                                                                   | Requires neural style transfer model (PyTorch/ONNX). Could use Stable Diffusion img2img or AdaIN-style transfer                                                                  |
| 6   | Generative fill / inpainting   | **Needs ML** | —                                                                                                   | Requires diffusion model (Stable Diffusion inpainting) or GAN. Heavy GPU inference                                                                                               |
| 7   | Voice-controlled editing       | **Needs ML** | —                                                                                                   | Requires speech recognition model (Whisper, Vosk, or cloud API). Command parsing is straightforward after transcription                                                          |
...
| 4   | Windows Ink / Pen       | **Out of scope** | —                  | Requires Windows-specific pointer event handling and pressure sensitivity APIs. Platform feature, not core functionality     |
| 5   | AppImage update (Linux) | **Out of scope** | —                  | Requires `electron-builder` AppImage target + update server (e.g., S3/GitHub Releases). Packaging/infrastructure task        |
...
| ~~17~~ | ~~NDI/Syphon/Spout native modules~~ | ~~Not relevant — post-production tool, not live VJ software~~ | ~~`utils/ndiOutput.ts`~~         |
...
| #   | Task                                                                                                    | Ticket      | Effort      | Priority             | Status   |
| --- | ------------------------------------------------------------------------------------------------------- | ----------- | ----------- | -------------------- | -------- |
| 1   | ~~Refactor `frameCache.ts` to use IPC (move fs ops to main process)~~                                   | ~~AUD-014~~ | ~~4 hours~~ | **~~P0 (Blocker)~~** | **DONE** |
| 2   | ~~Fix `useAudioReactive.ts` — remove setState from effect body~~                                        | ~~AUD-003~~ | ~~30 min~~  | ~~P1~~               | **DONE** |
| 3   | ~~Fix `useGPUInfo.ts` — remove setState from effect body~~                                              | ~~AUD-004~~ | ~~30 min~~  | ~~P1~~               | **DONE** |
| 4   | ~~Fix render queue stale closure — prefer job snapshot~~                                                | ~~AUD-015~~ | ~~1 hour~~  | ~~P1~~               | **DONE** |
| 5   | ~~Add proper MIDI event types, remove `any` from `midiControl.ts`~~                                     | ~~AUD-007~~ | ~~1 hour~~  | ~~P2~~               | **DONE** |
| 6   | ~~Add proper type for `memoryMonitor.ts` — remove `any`~~                                               | ~~AUD-006~~ | ~~15 min~~  | ~~P2~~               | **DONE** |
| 7   | ~~Remove unused variables from `cloudSync.ts` stubs~~                                                   | ~~AUD-005~~ | ~~5 min~~   | ~~P2~~               | **DONE** |
| 8   | ~~Change `let maxConcurrent` to `const` in `renderQueue.ts`~~                                           | ~~AUD-008~~ | ~~2 min~~   | ~~P3~~               | **DONE** |
| 9   | ~~Add `aria-label` to all `<select>` elements in `PropertiesPanel.tsx`~~                                | —           | 15 min      | P2                   | **DONE** |
| 10  | ~~Fix `EffectStack.tsx` — remove `Math.random()` / `Date.now()` impure calls during render~~            | —           | 10 min      | P2                   | **DONE** |
| 11  | ~~Fix `CommandPalette.tsx` — remove setState from effect body~~                                         | —           | 10 min      | P2                   | **DONE** |
| 12  | ~~Fix `oscDMX.ts` — remove unused eslint-disable directives, fix empty catch block~~                    | —           | 10 min      | P3                   | **DONE** |
| 13  | ~~Fix `oscDMX.unit.test.ts` — remove useless assignments~~                                              | —           | 5 min       | P3                   | **DONE** |
| 14  | ~~Fix `useSAM3.ts` — remove unused `imageData` and eslint-disable~~                                     | —           | 5 min       | P3                   | **DONE** |
| 15  | ~~Migrate inline styles to CSS classes / Tailwind (DebugOverlay, ExportPresets, RecentFiles, Toolbar)~~ | ~~AUD-010~~ | ~~4 hours~~ | ~~P2~~               | **DONE** |
| 16  | ~~Fix unused `decodeBase64Png` in `WebGLCanvas.tsx`~~                                                   | —           | 2 min       | P3                   | **DONE** |
| 17  | ~~Fix unused `loadModel` in `Viewport.tsx` (SAM integration)~~                                          | —           | 2 min       | P3                   | **DONE** |

**Agent 2 Total Estimated Effort:** ~11.5 hours (**~11.5h completed, 0h remaining**)

**Verification Status (as of latest run):**

- `npx tsc --noEmit`: **0 errors**
- `npx eslint src --ext .ts,.tsx`: **0 errors, 4 warnings** (react-hooks/exhaustive-deps in `StudioContext.tsx` — acceptable)

**All Agent 2 tasks are now complete.**
