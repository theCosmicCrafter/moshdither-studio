...
| 3   | Browser tests        | **Partial** | `WebGLCanvas.browser.test.tsx` | Config exists (`vitest.browser.config.ts`) but test still runs in jsdom. Needs Playwright Chromium runner invocation (`npx vitest --browser.name=chromium`). Not a code-writing task             |
...
| 2   | Auto-keyframe (music sync)     | **Stub**     | `utils/beatDetection.ts`                                                                     | Beat detection exists. Auto-keyframe generation from beat timestamps is a UI automation task, not ML. Straightforward to wire                      |
