...
| 5   | Real-time collaboration   | **Done**    | `electron/collaborationServer.ts`, `src/utils/collaboration.ts`, `src/components/organisms/CollaborationPanel.tsx` | WebSocket room server in main process. Effect stack sync via ops (add/remove/update/reorder). Presence tracking with avatars. Auto-start server on connect. Room ID generation                                                             |
