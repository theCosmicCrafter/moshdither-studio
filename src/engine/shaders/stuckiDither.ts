...
export const stuckiDitherShader: EffectShader = {
  id: "stucki_dither",
  name: "Stucki Dither",
  vertexSource: `
  ...
  fragmentSource: `
    ...
    float stuckiHash(vec2 p) {
      vec2 res = resolution.x > 0.0 ? resolution : vec2(1920.0, 1080.0);
      vec2 pix = p * res;
      float d = dot(pix, vec2(157.9, 281.3));
      float noise = fract(sin(d) * 43758.5453123);
      // Stucki 42nds dispersion across 12 neighbors
      return (noise - 0.5) * 0.88;
    }

    void main() {
      vec4 color = texture2D(tDiffuse, vUv);
      float lum = dot(color.rgb, vec3(0.299, 0.587, 0.114));
      float errNoise = stuckiHash(vUv) * amount;
