...
import type { Keyframe, EasingType } from "../store";
...
function makeKeyframe(time: number, value: number, easing: EasingType): Keyframe {
  return {
    id: `${time}-${value}-${Math.random().toString(36).slice(2, 7)}`,
    time,
    value,
    easing,
  };
...
function generatePulse(
  ...
  for (let i = 0; i < beats.length; i++) {
    const beat = beats[i];
    const intensity = Math.max(0, Math.min(1, Number.isNaN(beat.intensity) ? 0 : beat.intensity));
    const value = min + (max - min) * intensity;

    // Peak on the beat
    keyframes.push(makeKeyframe(beat.time, value, easing));
    ...
    if (i < beats.length - 1) {
      ...
      if (Number.isFinite(decayTime) && decayTime > beat.time) {
        keyframes.push(makeKeyframe(decayTime, min, "easeOut"));
      }
    } else {
      // Last beat: decay after
      keyframes.push(makeKeyframe(beat.time + decaySeconds, min, "easeOut"));
    }
  }
...
function generateToggle(beats: Beat[], min: number, max: number, easing: EasingType): Keyframe[] {
  const keyframes: Keyframe[] = [];

  for (let i = 0; i < beats.length; i++) {
    const value = i % 2 === 0 ? max : min;
    keyframes.push(makeKeyframe(beats[i].time, value, easing));
  }
...
function generateRamp(
  ...
  const keyframes: Keyframe[] = [];
  const safeCycle = cycleBeats > 0 ? cycleBeats : 1;
  const step = (max - min) / (safeCycle - 1 || 1);

  for (let i = 0; i < beats.length; i++) {
    const cyclePosition = i % safeCycle;
    const value = min + step * cyclePosition;
    keyframes.push(makeKeyframe(beats[i].time, Math.min(value, max), easing));
  }

  return keyframes;
}

function generateDecay(beats: Beat[], min: number, max: number, easing: EasingType): Keyframe[] {
  const keyframes: Keyframe[] = [];

  for (let i = 0; i < beats.length; i++) {
    const beat = beats[i];
    // Start high
    keyframes.push(makeKeyframe(beat.time, max, easing));

    // If there's a next beat, interpolate down to min before it
    if (i < beats.length - 1) {
      const nextBeat = beats[i + 1];
      const midTime = (beat.time + nextBeat.time) / 2;
      keyframes.push(makeKeyframe(midTime, min, "easeInOut"));
    }
  }
